...
2. 逐段分析（必须逐项覆盖，不得跳过）：
   a. 新增代码 → 
      - 分支完整性：是否正确处理了所有分支？if/else/switch 是否覆盖所有枚举值？是否有隐式 fall-through？
      - 边界条件：空值/零值/空集合/负数/最大值是否处理？循环条件是否正确？是否有 off-by-one？
      - 安全检查：是否存在注入风险（SQL/XSS/命令注入）？敏感数据是否加密/脱敏？权限检查是否完备？
      - 异常处理：是否捕获了所有可能的异常？异常是否丢失上下文？是否有静默失败（吞异常不处理）？
      - 资源管理：文件/连接/锁是否在异常路径下正确释放？
      - 死代码：新增的结构体/函数/导入是否被使用？是否有不可达分支？
      - 性能风险：是否有 N+1 查询？是否有不必要的大对象分配？是否有锁竞争？
      - 并发安全：是否存在竞态条件？共享状态是否正确同步？是否有死锁风险？
   b. 修改代码 → 
      - 调用方影响：签名是否变化？返回值类型是否变化？异常抛出是否变化？
      - 接口契约：是否破坏了向后兼容性？API 语义是否变化？
      - 逻辑替换：旧逻辑是否完全替换？是否有残留的旧代码路径？
      - 嵌套复杂度：是否引入了不必要的嵌套？是否可以用早返回简化？
   c. 删除代码 → 仅作上下文参考，不报告问题
   d. 新增/修改的文档字符串和注释 → 是否有拼写/语法错误或误导性描述？
...
| 级别 | 定义 | 示例 |
|------|------|------|
| high | 安全漏洞、数据丢失、崩溃、关键功能异常、并发竞态 | SQL 注入、空指针、签名变更未同步、竞态条件、资源泄露 |
| medium | 性能退化、可维护性问题、边界条件遗漏、接口兼容性破坏 | 缺少错误处理、N+1 查询、不必要耦合、API 语义变化 |
| low | 风格/命名问题（仅限违反规约时）、文档/注释拼写或语法错误、死代码/未使用导入 | 变量命名不一致、docstring 拼写错误、未使用的 import |
...
第 2~N 轮: 按 risk_zones 顺序调查（high → medium → low）
  对每个 risk_zone:
    ...
    3.5 假设验证：如果结论依赖某个行为假设（如"X API 在 Y 条件下会 Z"或"跨平台行为差异"），
        必须通过 Read 源码/文档验证该假设的实际行为，而非基于常识推断。
        无法验证 → 标记 uncertain，不确认。
...
**evidence.type：** `codegraph` | `file_read` | `diff` | `heuristic`

**evidence 质量规则：**
- `diff`：仅用于问题在 diff 中直接可见（硬编码值、命名错误、格式问题）
- 涉及行为推断（"X 会导致 Y"、"跨平台差异"、"安全漏洞可利用"）→ 必须 `codegraph` 或 `file_read` 验证
- Grep 回退找到的证据 → 标注 `heuristic`，severity 降一级
- `heuristic` 证据的 issue 在最终报告中标注 "⚠️ 建议人工复核"
- 禁止用常识替代验证："应该是这样" ≠ 证据
...
| 「根据常识，这个 API 在 Windows 上行为不同」 | 常识经常错。Read 源码/文档验证实际行为，或者标 uncertain |
| 「diff 里就能看出来，不用 codegraph 了」 | diff 只能看改了什么，不能看调用方/影响面。涉及跨文件判断必须用 codegraph |
| 「grep 找到了，就是 codegraph 证据」 | Grep 回退 = heuristic，不是 codegraph。不要伪装证据等级 |
