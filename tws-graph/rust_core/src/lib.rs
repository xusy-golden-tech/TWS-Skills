...
fn index_one_file<'conn>(
    conn: &'conn rusqlite::Connection,
    root_path: &Path,
    file_path: &Path,
    registry: &indexer::registry::Registry,
    insert_node: &mut rusqlite::CachedStatement<'conn>,
    insert_edge: &mut rusqlite::CachedStatement<'conn>,
    parser_pool: &mut indexer::parser_pool::ParserPool,
    ...
    // Parse with tree-sitter (reuse parser from pool)
    let parser = parser_pool.get_or_create(lang)?;
...
/// If `to_node` is provided, traverses inbound (callers); otherwise traverses outbound from `from_node`.
#[pyfunction]
#[pyo3(signature = (db_path, from_node=None, to_node=None, depth=None, kind=None, include_paths=None, exclude_paths=None))]
fn export_dot(db_path: &str, from_node: Option<&str>, to_node: Option<&str>, depth: Option<usize>, kind: Option<&str>, include_paths: Option<Vec<String>>, exclude_paths: Option<Vec<String>>) -> PyResult<String> {
    let db = open_db(db_path)?;
    ...
    Ok(export::export_dot(&db, from_node, to_node, depth.unwrap_or(2), kind, allowed_nodes.as_ref()))
}

/// Export a subgraph in Mermaid format.
/// If `to_node` is provided, traverses inbound (callers); otherwise traverses outbound from `from_node`.
#[pyfunction]
#[pyo3(signature = (db_path, from_node=None, to_node=None, depth=None, kind=None, include_paths=None, exclude_paths=None, group_by_file=None))]
fn export_mermaid(db_path: &str, from_node: Option<&str>, to_node: Option<&str>, depth: Option<usize>, kind: Option<&str>, include_paths: Option<Vec<String>>, exclude_paths: Option<Vec<String>>, group_by_file: Option<bool>) -> PyResult<String> {
    let db = open_db(db_path)?;
    let allowed_nodes = if has_scope(include_paths.as_ref(), exclude_paths.as_ref()) {
        let (inc, exc) = resolve_scope(&include_paths, &exclude_paths);
        Some(build_allowed_nodes(&db, inc, exc))
    ...
    Ok(export::export_mermaid(&db, from_node, to_node, depth.unwrap_or(2), kind, allowed_nodes.as_ref(), group_by_file.unwrap_or(false)))
}
