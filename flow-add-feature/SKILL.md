...
description: 添加新功能标准流程。复杂度门禁 → 简化或完整路径 → 逐任务(设计书→测试设计→编码→审查→测试执行→同步) → 集成测试
---
...
f. 代码审查（高风险任务时，主 agent 按 comp-code-review 编排层执行三阶段并行审查）
   审查发现问题先修再进测试——避免在已知有问题的代码上浪费测试时间
g. 测试执行（加载 `comp-test`，执行阶段：运行测试、检查边界、确认 0 failures）
   如测试失败：子 agent 按 comp-test 中的「先分类再行动」判定方向，不得直接改测试代码
h. 可选：目标回溯验证（满足触发条件时加载 `comp-goal-verify`，由独立审查子 agent 执行）
